...
function AlertViewComponent(props?: AlertViewProps, ref: React.Ref) {
  ...
  const scale = useSharedValue(SCALE_ANIMATION);
  ...
  const onDismissModal = useCallback(() => {
    ...
    progress.value = withTiming(0, { duration: 100, easing: Easing.linear }, () => {
  ...
  return (
    <ModalCustom
      ...
      zIndex={zIndex}
    >
      <Overlay showStatusBar={showStatusBar} progress={progress} />
                        ...
                        actions.map((btn, idx) => (
                          <React.Fragment key={String(idx)}>
                            ...
                            <Button
                              ...
                              title={btn.text}
                              onPress={() => {
                                btn.autoDismiss !== false && onDismissModal();
